...
export default function ProjectsPage() {
  ...
  // Shuffle projects when "All" filter is selected
  const filteredProjects = useMemo(() => {
    const filtered = filter === "All" 
      ? dbProjects 
      : dbProjects.filter((p: any) => p.category === filter);
    
    // Shuffle only when "All" is selected
    return filter === "All" ? shuffleArray(filtered) : filtered;
  }, [filter, dbProjects]);
